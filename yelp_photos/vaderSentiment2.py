...
class SentimentIntensityAnalyzer(object):
    ...
    def __init__(self, lexicon_file="vader_lexicon.txt", emoji_lexicon="emoji_utf8_lexicon.txt"):
        ...
        with open('/usr/local/lib/python3.6/site-packages/vaderSentiment/vader_lexicon.txt',encoding='utf-8') as f:
            self.lexicon_full_filepath = f.read()
        self.lexicon = self.make_lex_dict()

        #emoji_full_filepath = os.path.join(os.path.dirname(_this_module_file_path_), emoji_lexicon)
        with open('/usr/local/lib/python3.6/site-packages/vaderSentiment/emoji_utf8_lexicon.txt', encoding='utf-8') as f:
...
if __name__ == '__main__':
    ...
    revi = open("sentiment_reviews.txt","w")
    ...
    for i in data:
        ...
        sen = 0
        sentiment = analyzer.polarity_scores(caption_text)
        if caption_text=="":
        ...
        if sentiment['compound']>=0.05  or sentiment['compound']<=-0.05:
            if sentiment['compound']>=0.05:
                ...
                x = "Negative"
            print("Caption_text is : "+str(caption_text)+" polarity : "+x) 
        '''
        if sentiment['compound']>-0.05 and sentiment['compound']<0.05:
            print("Caption_text is : "+str(caption_text)+" polarity : "+"Neutral")     
    revi.close()
    file_count = 0
    for line in open('sentiment_reviews.txt').readlines(  ): file_count += 1
    print("No of lines in the file are :"+str(file_count))
    print("Total no of images are : "+str(total_no_of_images))
    print("")
    
    print("No of captions with empty text are : "+str(empty_caption_count))
    print("")
	
    print("The sentiment statistics of no of positve, negative, neutral are as follows :")
    print(sentiments)
    print("")
